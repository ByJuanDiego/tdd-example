...
def test_the_results_is_correct_for_simple_cases():
    try:
        ret = client.get(f"/getcoordinates/?city={test_city.name}")
        ret = ret.json()
        assert (abs(test_city.lat - float(ret["latitude"])) < epsilon) and (
            abs(test_city.lon - float(ret["longitude"])) < epsilon
...
def test_the_result_is_correct_for_all_inputs_get_coordinates(
    ...
    try:
        ...
        expected = {"longitude": expected_lon, "latitude": expected_lat}
...
def test_get_distance_is_correct_for_1_case():
    try:
        ret = client.get(
            f"/getdistance/?lat1={lat1}&lon1={lon1}&lat2={lat2}&lon2={lon2}"
        )
        ret = ret.json()

        expected_distance = 5585.23357
        assert (
            abs(ret["distance"] - expected_distance) < epsilon
        ), "The computed distance is not correct"
